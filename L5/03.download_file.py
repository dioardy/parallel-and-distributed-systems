...
# nymimpan URL di variabel url
url = "https://apod.nasa.gov/apod/image/1901/LOmbradellaTerraFinazzi.jpg"

# fungsi untuk setiap threadnya membangun range byte
...
def main(url=None, splitBy=3):
    ...
    # request head dari url untuk mendapatkan ukuran byte file
    sizeInBytes = requests.head(url, headers={'Accept-Encoding': 'identity'}).headers.get('content-length', None)
    print("%s bytes to download." % sizeInBytes)
    
    # cek apakah variabel sizeInBytes menyimpan nilai selain Null
    # sizeInBytes Null = keluar fungsi
    if not sizeInBytes:
